...
const eachDateInRange = (startKey: string, endKey: string) => {
...
const buildHeaderLabel = (dateKey: string, todayKey: string) => {
...
const Timeline = () => {
  const [rawTasks, setRawTasks] = useState<ScheduledTask[]>([]);
  const [loading, setLoading] = useState(true);
  const [toggling, setToggling] = useState(false);
  const [today, setToday] = useState(getToday());
  const [activeIndex, setActiveIndex] = useState(0);
  const [isAnimating, setIsAnimating] = useState(false);
  const [transitionMeta, setTransitionMeta] = useState<{ prevIndex: number | null; direction: number; running: boolean }>({ prevIndex: null, direction: 0, running: false });
  const [editTask, setEditTask] = useState<ScheduledTask | null>(null);
  const [editForm, setEditForm] = useState<TimelineEditForm>({ task_description: '', scheduled_date: '', estimated_minutes: '' });
  ...
  const fetchTimeline = useCallback(async () => {
    try {
      const tasks = await apiRequest<ScheduledTask[]>('/api/timeline', { method: 'GET', headers: {} });
      setRawTasks(tasks);
    } catch (error) {
  ...
  const dateLayers = useMemo(() => {
    const grouped = visibleTasks.reduce<Record<string, ScheduledTask[]>>((acc, task) => {
  ...
  const stepDay = useCallback((direction: number) => {
  ...
  const maybeAutoAdvance = useCallback((nextTasks: ScheduledTask[], activeDate?: string) => {
    if (!activeDate) return;
    const dayTasks = nextTasks.filter((task: ScheduledTask) => task.scheduled_date === activeDate);
    if (dayTasks.length === 0) return;
    if (dayTasks.every((task: ScheduledTask) => !!task.completed)) {
      stepDay(1);
    }
  }, [stepDay]);
