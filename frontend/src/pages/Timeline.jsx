    ...
    <div className="flex-1">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {tasks.map((task, i) => (
                <div 
                    key={i} 
                    className={`relative p-4 rounded-lg border-l-4 shadow-sm bg-white hover:shadow-md transition-all ${
                        task.completed ? 'opacity-60 grayscale' : ''
                    } ${
                        task.complexity === 'Hard' ? 'border-l-red-500' : 
                        task.complexity === 'Medium' ? 'border-l-yellow-400' : 'border-l-blue-400'
                    }`}
                >
                    <div className="flex justify-between items-start mb-1">
                         <h4 className={`font-bold text-sm ${task.completed ? 'text-slate-500 line-through' : 'text-slate-700'}`}>
