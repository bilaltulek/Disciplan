...
const TaskCard = ({ task }) => {
  ...
  const toggleTask = async (taskId, currentStatus) => {
    ...
    const newPlan = plan.map(t => t.id === taskId ? { ...t, completed: !isComplete } : t);
    setPlan(newPlan);
                ...
                {loading ? <div className="text-center py-10">Loading...</div> : (
                    plan.map((step, i) => (
                        <div key={i} className={`flex items-start gap-3 p-3 rounded-lg border transition-colors ${step.completed ? 'bg-slate-50 border-slate-100' : 'bg-white border-slate-200'}`}>
                            <button onClick={() => toggleTask(step.id, step.completed)} className={`mt-1 w-5 h-5 rounded border flex items-center justify-center transition-colors ${!step.completed ? 'bg-green-500 border-green-500 text-white' : 'border-slate-300 hover:border-blue-500'}`}>
